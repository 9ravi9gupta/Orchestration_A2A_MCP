...
class ACPCallingAgent(MultiStepAgent):
    ...
    def __init__(
        ...
        if prompt_templates is None:
            prompt_templates = {
                ...
                3. When you have a complete answer, ALWAYS call the final_answer tool with your final response
                ...
                Remember:
                - Always use the final_answer tool when you have a complete answer after all task completion
            ...
            # example_tool_use = """
            # Example:
            # User: What is (3+5)*(12*3)?
            # Assistant: [calls calculation_agent]
            # Observation: The result is 288.
            # Assistant: [calls final_answer with {"answer": "The result is 288."}]
            # """
            # prompt_templates["system_prompt"]+=example_tool_use
        ...
        for name, agent in acp_agents.items():
            ...
            acp_tools[name].__call__ = make_caller(name,
                                                   agent['client'])  # This line isn't actually using the agent's client

        # Add final_answer tool
        acp_tools["final_answer"] = Tool(
            name="final_answer",
            description="It should only be called at the end after you get final result. It take the final answer to the user's query as input",
            inputs={"answer": "The final result of user query after getting final result"},
            output_type="str"
        )
    ...
    async def step(self, memory_step: ActionStep) -> Union[None, Any]:
        ...
        system_msgs = list(filter(lambda x: x['role'] == 'system', memory_messages))
        other_msgs = list(filter(lambda x: x['role'] != 'system', memory_messages))
        memory_messages = system_msgs + other_msgs
        ...
        try:
            model_message: ChatMessage = self.model(
                ...
                stop_sequences=["Observations:", "Calling agent:"],
            )
        ...
        if not hasattr(model_message, 'tool_calls') or model_message.tool_calls is None or len(
                model_message.tool_calls) == 0:
            ...
            if model_message.content and "final_answer" in model_message.content.lower():
                ...
                if "tool:" in content.lower() or "agent:" in content.lower():
                    try:
                        ...
                        lines = content.split('\n')
                        tool_line = next(
                            (line for line in lines if "tool:" in line.lower() or "agent:" in line.lower()), None)
                        if tool_line:
                            ...
                            if len(parts) > 1:
                                ...
                                memory_step.tool_calls = [
                                    ToolCall(name=agent_name, arguments=agent_arguments, id="synthetic_id")]
    ...
    async def _process_tool_call(self, memory_step: ActionStep, agent_name: str, agent_arguments: Any) -> Union[
        None, Any]:
        """
        ...
        if agent_name == "final_answer":
            ...
            #-----custom code change-----------------
            if agent_name == 'termination_agent':
                if isinstance(agent_arguments, str):
                    final_answer = json.loads(agent_arguments).get('input', 'Not Able to Answer the Questions. Ask Another one')
                elif isinstance(agent_arguments, dict):
                    final_answer = agent_arguments.get('input', 'Not Able to Answer the Questions. Ask Another one')
                else:
                    pass
        ...
        for step_num in range(max_steps):
            ...
            try:
                ...
                if hasattr(memory_step, 'observations') and memory_step.observations:
                    ...
                    #--------custom code changes and integration-------------------------
                    # Add observation as user message
                    observation_text = memory_step.observations
                    if isinstance(observation_text, dict) and "output" in observation_text:
                        observation_text = observation_text["output"]
                    #------------------------------------------------

                    self.input_messages.append({
                        "role": "user",
                        "content": [{"type": "text", "text": f"Observation: {observation_text}"}]
                    })

                    #-------custom code integrations
                    system_msgs = list(filter(lambda x: x['role'] == 'system', self.input_messages))
                    other_msgs = list(filter(lambda x: x['role'] != 'system',  self.input_messages))
                    self.input_messages = system_msgs + other_msgs
